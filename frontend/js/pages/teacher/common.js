// Загрузка компонентов и подсветка активного пункта меню
async function loadTeacherComponents(activePath) {
    const [navHtml, sideHtml] = await Promise.all([
        fetch('/components/navbar.html').then(r => r.text()),
        fetch('/components/sidebar-teacher.html').then(r => r.text()),
    ]);
    ...
    if (activePath) {
        document.querySelectorAll('.nav-sidebar .nav-link').forEach(link => {
            if (link.getAttribute('href') && link.getAttribute('href').includes(activePath)) {
